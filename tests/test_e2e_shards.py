...
def test_repository_manifest_covers_every_e2e_file_once_and_550_cases():
    manifest = load_manifest(MANIFEST)
    validated = validate_manifest(manifest, E2E_DIR)

    assert manifest["expected_total"] == EXPECTED_TOTAL == 550
...
def test_workflow_runs_safe_non_retrying_shards_and_an_always_gate():
    ...
    assert "--expected-total 550" in workflow
